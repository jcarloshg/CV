# Job Description

## Role
Desarrollador Angular Sr

## Company
Skala - Fintech (Visa Card Issuing)

## Job Description
En Skala trabajamos con una empresa de tecnología de pagos del sector Fintech en la búsqueda de un/a Software Engineer especializado en Visa Card Issuing

El rol tiene como objetivo liderar el desarrollo e implementación de la solución de emisión de tarjetas Visa dentro de una plataforma de pagos en crecimiento.

Lo que buscamos:
- Experiencia experta en Visa Card Issuing
- Dominio de C# y C++
- Trabajo en equipos ágiles bajo metodología Scrum
- Inglés intermedio o superior

## Location
Mexico (Remote)

## Highlights
- Fintech / Payments industry
- Visa Card Issuing specialization
- Senior Angular Developer
- C# and C++ expertise required
- Scrum methodology